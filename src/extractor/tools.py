    ...
    for root, dirs, files in os.walk(folder_name):
        for index, file in enumerate(files):
            if not file.endswith(".java"):
                logger.debug('Removing non .java file: {}'.format(os.path.join(root, file)))
                os.remove(os.path.join(root, file))
            elif os.path.getsize(os.path.join(root, file)) == 0:
                logger.debug('Removing empty file: {}'.format(os.path.join(root, file)))
                os.remove(os.path.join(root, file))

    logger.debug('Total files remaining: {}'.format(sum([len(files) for r, d, files in os.walk(folder_name)])))