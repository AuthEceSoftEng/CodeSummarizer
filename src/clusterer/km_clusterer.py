...
class KMClust(AbsClust):
    ...
    def __init__(self, vectorizer, labels_true, init='k-means++',
                 max_iter=1000, n_init=10, tol=1e-6):

        super(KMClust, self).__init__('KMeans', vectorizer, labels_true)
        self.init = init
    ...
    def cluster(self, n_clusters):
        ...
        km_clusterer = KMeans(n_clusters=n_clusters, init=self.init, max_iter=self.max_iter, n_init=self.n_init,
                              verbose=False, n_jobs=4, tol=self.tol)

        logger.info('Clustering sparse data with {}'.format(km_clusterer))

        t0 = time()
        km_clusterer.fit(self.vectorizer.vec_data)
    ...
    def metrics(self, show_graphs=False, search=False):
        ...
        results[0] = metrics.homogeneity_score(self.labels_true, self.labels_pred)
        results[1] = metrics.completeness_score(self.labels_true, self.labels_pred)
        results[2] = metrics.v_measure_score(self.labels_true, self.labels_pred)
        results[3] = metrics.adjusted_rand_score(self.labels_true, self.labels_pred)
        results[4] = metrics.silhouette_score(self.vectorizer.vec_data, self.labels_pred, sample_size=1000)
        coh_sep = cohesion_separation_per_cluster(self.vectorizer.vec_data, self.labels_pred, self.n_clusters,
                                                  self.cluster_centers, show_graphs=False)
        ...
        if show_graphs:
            ...
            #  silhouette_per_cluster(self.vectorizer.vec_data,
                                   #  self.labels_pred,
                                   #  self.n_clusters)
            cohesion_separation_per_cluster(self.vectorizer.vec_data, self.labels_pred,
                                            self.n_clusters, self.cluster_centers,
    ...
    def top_terms_per_cluster(self, num=15, show=False):
        ...
        for i in range(len(self.cluster_centers)):
            top_terms.append([self.vectorizer.feature_names[ind] for ind in order_centroids[i, :num]])
        if show:
